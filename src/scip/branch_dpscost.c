...
#define BRANCHRULE_NAME          "dpscost"
#define BRANCHRULE_DESC          "branching on discounted pseudo cost values"
...
#define BRANCHRULE_DISCOUNT_FACTOR        0.1 /**< default discount factor for discounted pseudo costs.*/
   ...
   )
{
   ...
   else
   {
      pscostdown  = SCIPgetVarPseudocostVal(scip, cand, -deltaminus) 
                  + branchruledata->discountfactor * SCIPgetVarDPseudocostVal(scip, cand, -deltaminus);
      pscostup    = SCIPgetVarPseudocostVal(scip, cand,  deltaplus) 
                  + branchruledata->discountfactor * SCIPgetVarDPseudocostVal(scip, cand, deltaplus);
...
static
SCIP_DECL_BRANCHCOPY(branchCopyDPscost)
{  /*lint --e{715}*/
   assert(scip != NULL);
...
static
SCIP_DECL_BRANCHFREE(branchFreeDPscost)
{  /*lint --e{715}*/
   SCIP_BRANCHRULEDATA* branchruledata;
...
static
SCIP_DECL_BRANCHINIT(branchInitDPscost)
{  /*lint --e{715}*/
   SCIP_BRANCHRULEDATA* branchruledata;
...
static
SCIP_DECL_BRANCHEXECLP(branchExeclpDPscost)
{  /*lint --e{715}*/
   SCIP_BRANCHRULEDATA* branchruledata;
   SCIP_VAR** lpcands;
   ...
   SCIPdebugMsg(scip, "Execlp method of dpscost branching\n");
   ...
   for( c = 0; c < nlpcands; ++c )
   {
      ...
      score = SCIPgetVarDPseudocostScore(scip, lpcands[c], lpcandssol[c], 
                                         branchruledata->discountfactor);
...
static
SCIP_DECL_BRANCHEXECEXT(branchExecextDPscost)
{  /*lint --e{715}*/
   SCIP_BRANCHRULEDATA* branchruledata;
   ...
   SCIPdebugMsg(scip, "Execext method of dpscost branching\n");
   ...
   )
{
   ...
   /* create dpscost branching rule data */
   ...
   SCIP_CALL( SCIPsetBranchruleCopy(scip, branchrule, branchCopyDPscost) );
   SCIP_CALL( SCIPsetBranchruleFree(scip, branchrule, branchFreeDPscost) );
   SCIP_CALL( SCIPsetBranchruleInit(scip, branchrule, branchInitDPscost) );
   SCIP_CALL( SCIPsetBranchruleExecLp(scip, branchrule, branchExeclpDPscost) );
   SCIP_CALL( SCIPsetBranchruleExecExt(scip, branchrule, branchExecextDPscost) );
   ...
   SCIP_CALL( SCIPaddRealParam(scip, "branching/" BRANCHRULE_NAME "/discountfactor",
         "discount factor for discounted pseudo costs.",
         &branchruledata->discountfactor, FALSE, BRANCHRULE_DISCOUNT_FACTOR, 0.0, 1.0, NULL, NULL) );
